...
def test_group_filter_selects_only_that_company():
    entries, fuzzy, not_found = H._filter_entries(ENTRIES, ["KZO"], [])
...
def test_group_filter_multiple_groups_is_union():
    entries, fuzzy, not_found = H._filter_entries(ENTRIES, ["KZP", "S5"], [])
...
def test_name_filter_exact_selects_only_that_wallet():
    entries, fuzzy, not_found = H._filter_entries(ENTRIES, [], ["KZP 96G1"])
...
def test_name_filter_fuzzy_resolves_a_typo_and_is_recorded():
    ...
    entries, fuzzy, not_found = H._filter_entries(ENTRIES, [], ["KZP 96G2"])
    assert {e["name"] for e in entries} == {"KZP 96G1"}
...
def test_name_not_found_is_reported_not_silently_dropped_or_guessed():
    entries, fuzzy, not_found = H._filter_entries(ENTRIES, [], ["ZZZ QQQ"])
    assert entries == []
...
def test_group_and_name_together_is_an_intersection():
    ...
    entries, fuzzy, not_found = H._filter_entries(ENTRIES, ["KZO"], ["Eth One"])
    ...
    entries2, fuzzy2, not_found2 = H._filter_entries(ENTRIES, ["KZO"], ["KZP 96G1"])
    assert entries2 == []
